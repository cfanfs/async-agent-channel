...
aac send --to alice "见附件" --file ~/aac-workspace/shared/a.txt --file ~/aac-workspace/shared/b.txt
```
...
aac inbox              # 列出未处理消息
aac inbox read <id>    # 读取消息并显示附件落盘路径
...
收到的附件会落到 `~/aac-workspace/received/<message-id>/`（或你配置的 inbound 工作区）下面。
