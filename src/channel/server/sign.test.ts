...
describe("buildSigningString", () => {
  it("joins parts with newlines", () => {
    const result = buildSigningString("POST", "/api/v1/messages", "2026-04-03T00:00:00Z", '{"to":"bob"}');
...
describe("signRequest", () => {
  it("returns keyId, timestamp, and signature", () => {
...
describe("verifySignature", () => {
  it("accepts a valid signature", () => {
    ...
    const body = '{"to":"bob"}';
    const sigStr = buildSigningString("POST", "/api/v1/messages", timestamp, body);
    const sig = computeSignature(sigStr, TEST_USER_ID);

    expect(
      verifySignature("POST", "/api/v1/messages", timestamp, body, TEST_USER_ID, sig)
    ).toBe(true);
  });
  ...
  it("rejects wrong signature", () => {
    const timestamp = new Date().toISOString();
    expect(
      verifySignature("POST", "/path", timestamp, "body", TEST_USER_ID, "bad-signature-value-0000000000000000000000000000000000")
    ).toBe(false);
  });
  ...
  it("rejects expired timestamp", () => {
    ...
    const body = "test";
    const sigStr = buildSigningString("GET", "/path", old, body);
    const sig = computeSignature(sigStr, TEST_USER_ID);

    expect(
      verifySignature("GET", "/path", old, body, TEST_USER_ID, sig)
    ).toBe(false);
  });

  it("rejects tampered body", () => {
    const timestamp = new Date().toISOString();
    const sigStr = buildSigningString("POST", "/path", timestamp, "original");
    const sig = computeSignature(sigStr, TEST_USER_ID);

    expect(
      verifySignature("POST", "/path", timestamp, "tampered", TEST_USER_ID, sig)
    ).toBe(false);
  });

  it("rejects invalid timestamp format", () => {
    expect(
      verifySignature("GET", "/path", "not-a-date", "", TEST_USER_ID, "a".repeat(64))
    ).toBe(false);
  });
});
